...
const SECURITY_CONFIG = {
  ...
  BLOCKED_DOMAINS: ['mailinator.com', 'tempmail.org', '10minutemail.com', 'guerrillamail.com'], // Domaines temporaires
  ALLOWED_DOMAINS: ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'zalamagn.com'], // Domaines autorisés
...
function validateSecurity(email: string, message: string, subject: string, ip: string): { valid: boolean; error?: string } {
  ...
  if (SECURITY_CONFIG.BLOCKED_DOMAINS.some(domain => emailDomain.includes(domain))) {
  ...
  if (message.length > SECURITY_CONFIG.MAX_MESSAGE_LENGTH) {
    return { valid: false, error: `Le message ne peut pas dépasser ${SECURITY_CONFIG.MAX_MESSAGE_LENGTH} caractères` };
  }
  
  if (message.length < SECURITY_CONFIG.MIN_MESSAGE_LENGTH) {
    return { valid: false, error: `Le message doit contenir au moins ${SECURITY_CONFIG.MIN_MESSAGE_LENGTH} caractères` };
  }
  
  // Vérifier la longueur du sujet
  if (subject.length > SECURITY_CONFIG.MAX_SUBJECT_LENGTH) {
    return { valid: false, error: `Le sujet ne peut pas dépasser ${SECURITY_CONFIG.MAX_SUBJECT_LENGTH} caractères` };
  }
  
  if (subject.length < SECURITY_CONFIG.MIN_SUBJECT_LENGTH) {
    return { valid: false, error: `Le sujet doit contenir au moins ${SECURITY_CONFIG.MIN_SUBJECT_LENGTH} caractères` };
  }
  
  // Vérifier le contenu du message (anti-spam basique)
  const spamKeywords = ['viagra', 'casino', 'loan', 'credit', 'make money fast', 'click here'];
  ...
  if (userLimit) {
    if (now < userLimit.resetTime) {
      if (userLimit.count >= SECURITY_CONFIG.MAX_REQUESTS_PER_HOUR) {
        const remainingMinutes = Math.ceil((userLimit.resetTime - now) / (1000 * 60));
        return { valid: false, error: `Trop de messages envoyés. Réessayez dans ${remainingMinutes} minutes` };
      }
      userLimit.count++;
    } else {
      rateLimitMap.set(userKey, { count: 1, resetTime: now + (SECURITY_CONFIG.COOLDOWN_MINUTES * 60 * 1000) });
    }
  } else {
    rateLimitMap.set(userKey, { count: 1, resetTime: now + (SECURITY_CONFIG.COOLDOWN_MINUTES * 60 * 1000) });
  }
